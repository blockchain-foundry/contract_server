...
Apply Action Helper
'''
def apply_deploy_contract(contract_file, contract_name):
    print('[Apply Deploy Contract] {}:{}'.format(contract_file, contract_name))
...
def apply_deploy_sub_contract(contract_file, contract_name, multisig_address, deploy_address, source_code):
    print('[Apply Deploy SubContract] {}:{} @{}/{}'.format(contract_file, contract_name, multisig_address, deploy_address))
    # 1. Create a contract
...
def apply_transaction_call_contract(contract_address, function_name, function_inputs, from_address):
    print('[Call Contract Function] {}({}) @{}'.format(function_name, function_inputs, contract_address))
...
def apply_call_constant_contract(contract_address, function_name, function_inputs, from_address):
    ...
    r_json_apply_transaction_call_contract = callContractFunction(contract_address, data)
    function_outputs = r_json_apply_transaction_call_contract['function_outputs']
    return function_outputs


def apply_call_sub_contract(contract_address, deploy_address, function_name, function_inputs):
    print('[Call SubContract Function] {}({}) @{}/{}'.format(function_name, function_inputs, contract_address, deploy_address))
    data = {
        'from_address': owner_address,
        'amount': '0',
        'color': '0',
...
def apply_watch_event(contract_address, key, oracle_url, callback_url, receiver_address):
    print('[Watching Event] key:{} @{}/{}'.format(key, contract_address, receiver_address))

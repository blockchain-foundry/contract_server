...
func (self *StateTransition) buyGas() error {
	...
	if sender.Balance(0).Cmp(mgval) < 0 {
		return fmt.Errorf("insufficient ETH for gas (%x). Req %v, has %v", sender.Address().Bytes()[:4], mgval, sender.Balance(0))
	}
...
func (self *StateTransition) TransitionDb() (ret []byte, requiredGas, usedGas *big.Int, err error) {
	...
	if contractCreation {
		ret, _, err = vmenv.Create(sender, self.data, self.gas, self.gasPrice, common.NewBalance(self.value,self.color))
		...
		ret, err = vmenv.Call(sender, self.to().Address(), self.data, self.gas, self.gasPrice, common.NewBalance(self.value,self.color))
		if err != nil {
...
func (self *StateTransition) refundGas() {
	...
	sender.AddBalance(0,remaining)

	// Apply refund counter, capped to half of the used gas.
	uhalf := remaining.Div(self.gasUsed(), common.Big2)
	refund := common.BigMin(uhalf, self.state.GetRefund())
	self.gas.Add(self.gas, refund)
	self.state.AddBalance(0,sender.Address(), refund.Mul(refund, self.gasPrice))

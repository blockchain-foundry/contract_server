...
func opCallValue(instr instruction, pc *uint64, env Environment, contract *Contract, memory *Memory, stack *stack) {
	...
	color :=stack.pop()
	var value *big.Int
	if val, ok := contract.Value()[uint(color.Uint64())]; ok {
...
func opCall(instr instruction, pc *uint64, env Environment, contract *Contract, memory *Memory, stack *stack) {
	...
	ret, err := env.Call(contract, address, args, gas, contract.Price, common.NewBalance(value,uint(color.Uint64())))
...
func opSuicide(instr instruction, pc *uint64, env Environment, contract *Contract, memory *Memory, stack *stack) {
	balance := env.Db().GetBalance(0,contract.Address())
	env.Db().AddBalance(0,common.BigToAddress(stack.pop()), balance)

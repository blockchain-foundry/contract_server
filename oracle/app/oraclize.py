...
def deployOraclizeContract(multisig_address, oraclize_address, byte_code): 
    ...
    command = EVM_PATH + ' --code ' + byte_code + ' --deploy ' + ' --read ' + CONTRACT_PATH + ' --receiver ' + oraclize_address + ' --write ' + CONTRACT_PATH
...
def set_var_oraclize_contract(multisig_address, oraclize_address, variable):  
    ...
    command = EVM_PATH + ' --read ' + CONTRACT_PATH + ' --input ' + INPUT + ' --receiver ' + oraclize_address + ' --write ' + CONTRACT_PATH
    check_call(command, shell=True)     

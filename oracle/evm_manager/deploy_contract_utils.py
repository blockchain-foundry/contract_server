...
def deploy_to_evm(sender_addr, multisig_address, byte_code, value, is_deploy, contract_address, tx_hash, ex_tx_hash):
    ...
    if is_deploy:
        command = EVM_PATH + " --sender " + sender_hex + " --fund " + "'" + value + "'" + " --value " + "'" + value + "'" + \
            ...
            byte_code + " --receiver " + contract_address + " --time " + str(_time) + " --read " + contract_path
        ...
        for link in links:
            ...
            set_var_oraclize_contract(multisig_address, link.oraclize_contract.address, info)
...
def deploy_single_tx(tx_hash, ex_tx_hash, multisig_address):
    ...
    if tx['type'] == 'CONTRACT':
        try:
            sender_addr, multisig_address, contract_address, bytecode, value, is_deploy, blocktime = get_contracts_info(
                tx)
        except:
        ...
        try:
            completed, status, message = deploy_to_evm(
                sender_addr, multisig_address, bytecode, value, is_deploy, contract_address, tx_hash, ex_tx_hash)
